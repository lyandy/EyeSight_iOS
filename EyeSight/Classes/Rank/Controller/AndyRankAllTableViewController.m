//
//  AndyRankAllTableViewController.m
//  EyeSight
//
...
@interface AndyRankAllTableViewController ()<MJRefreshBaseViewDelegate>
...
@implementation AndyRankAllTableViewController
...
- (void)loadNewData
{
    [AndyHttpTool getWithURL:@"http://baobab.wandoujia.com/api/v1/ranklist?strategy=historical" params:nil success:^(id json) {
        
        [self CommbineNewData:json];
        
        [[NSString stringWithFormat:@"%@", json] saveContentToFile:RankAllCacheFileName atomically:YES];
        
    } failure:^(NSError *error) {
        [MBProgressHUD hideHUDForView:XAppDelegate.rankViewController.navigationController.view];
        NSString *cacheFilePath = [AndyCommonFunction getCacheFilePathWithFileName:RankAllCacheFileName];
...
{
    AndyRankAllListModel  *listModel = [AndyRankAllListModel objectWithKeyValues:json];
