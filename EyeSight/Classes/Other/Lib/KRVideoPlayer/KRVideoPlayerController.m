...
@property (nonatomic, assign) BOOL isPlayingDownloadVideo;
...
- (void)setContentURL:(NSURL *)contentURL
{
    ...
    if([contentURL.description containsString:@"file://"])
    {
        if([AndyCommonFunction checkFileIfExist:[contentURL.description stringByReplacingOccurrencesOfString:@"file://" withString:@""]])
        {
            self.isPlayingDownloadVideo = YES;
            
            AndyLog(@"播放本地视频");
        }
        else
        {
            self.isPlayingDownloadVideo = NO;
            AndyLog(@"播放网络视频");
        }
    }
    else
...
- (void)onMPMoviePlayerPlaybackStateDidChangeNotification
{
    if (self.playbackState == MPMoviePlaybackStatePlaying) {
        ...
        if (self.qualityArray.count > 1 && !self.isPlayingDownloadVideo)
        {
            self.videoControl.qualityButton.enabled = YES;
        }
        else
        {
            self.videoControl.qualityButton.enabled = NO;
        }
        ...
        [self stopDurationTimer];
        if(!self.videoControl.lockButton.isSelected)
        {
